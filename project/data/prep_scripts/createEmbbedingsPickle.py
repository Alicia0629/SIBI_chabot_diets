...
def save_embeddings_to_pickle(embeddings, filename='data/datasets/embeddings.pkl'):
    with open(filename, 'wb') as f:
        pickle.dump(embeddings, f)
    print(f"Embeddings saved in {filename}")

def main():
    file_path = 'data/datasets/NewRecipes.csv' 
    df = load_recipes(file_path)  
    embeddings = generate_embeddings(df) 
    save_embeddings_to_pickle(embeddings) 
